...
with engine.connect() as conn:
    ...
    for target in developers_to_verify:
        ...
        # On vérifie si le dev existe par email
        ...
        if db_dev:
            ...
            print(f"✅ Trouvé: {db_name} ({email})")
            
            # Mise à jour si nécessaire
            needs_update = False
            updates = {}
            if db_username != target["gitlab_username"]:
                print(f"  ⚠️ Username GitLab incorrect: DB='{db_username}', Cible='{target['gitlab_username']}'")
                updates["u"] = target["gitlab_username"]
                needs_update = True
            
            if db_name != target["name"]:
                print(f"  ⚠️ Nom incorrect: DB='{db_name}', Cible='{target['name']}'")
            ...
            if needs_update:
                ...
                print(f"  ⚡ Données synchronisées avec succès pour {email}.")
        else:
            print(f"❌ MANQUANT: {target['name']} ({email}). Création requise via l'interface ou import CSV.")
