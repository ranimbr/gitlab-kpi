...
def create_scheduler() -> AsyncIOScheduler:
    ...
    logger.info(
        "Scheduler configured — last day of month 20:00 UTC"
    )
...
async def _run_monthly_job() -> None:
    ...
    """
    Job mensuel Scheduler.

    Workflow :

    1. Close period
    2. Extraction MONTHLY
    3. Cleanup REALTIME
    4. KPI snapshots
    """
    ...
    try:
        ...
        logger.info(
            f"[Scheduler] Monthly job success : {result}"
        )

    except Exception as e:

        logger.error(
            f"[Scheduler] Monthly job failed : {e}",
            exc_info=True
        )
