...
class GitLabClient:
    ...
    Support multi-tenant via GitLabConfig.

    Corrections :
    - Déchiffrement token avant usage
    - Pagination automatique
    - Endpoint approvals MR dédié
    """
    ...
    def _decrypt_token(token: str) -> str:
        """
        Déchiffre le token GitLab stocké chiffré.

        Si decrypt_token n'existe pas → fallback token brut.
        """
        ...
        try:
            ...
            logger.warning(
                "decrypt_token unavailable — using raw token"
            )
    ...
    async def get_merge_request_approvals(
        ...
        try:
            ...
            logger.warning(
                f"Approvals unavailable MR={mr_iid}: {e}"
            )

            return None

    # ───────────────── approval state (GitLab EE)
    ...
    async def get_merge_request_approval_state(
        ...
        try:
            ...
            logger.warning(
                f"approval_state unavailable MR={mr_iid}: {e}"
            )
