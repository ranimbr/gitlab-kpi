...
from app.models.period import PeriodStatusEnum   # ✅ CORRECTION POINT 9
...
def create_period(
    ...
    if existing:
        raise HTTPException(
            ...
            detail=f"Period {request.year}/{request.month:02d} already exists"
        )
...
def close_period(
    ...
    # ✅ CORRECTION POINT 9 — Comparaison enum au lieu de string
    # Avant : period.status == "closed"  → comparaison string fragile
    # Après : period.status == PeriodStatusEnum.closed  → correct et typé
    if period.status == PeriodStatusEnum.closed:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Period {period.year}/{period.month:02d} is already closed"
        )

    repo.close_period(db, period)
    db.commit()
    db.refresh(period)
