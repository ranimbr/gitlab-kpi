...
const toastOk = (msg = "Mot de passe réinitialisé avec succès") => Swal.fire({
  html: `<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px 0"><div style="width:76px;height:76px;border-radius:50%;background:rgba(16,185,129,.08);display:flex;align-items:center;justify-content:center;border:1px solid rgba(16,185,129,.25);box-shadow:0 0 32px rgba(16,185,129,.2)"><i class="ri-lock-check-line" style="font-size:2.2rem;color:#10B981"></i></div><div style="text-align:center"><p style="margin:0 0 4px;font-size:1.05rem;font-weight:700;color:#fff">Succès</p><p style="margin:0;color:rgba(255,255,255,.4);font-size:.875rem">${msg}</p></div></div>`,
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    // Load fonts
    if (!document.getElementById("rp3-fonts")) {
      const l = Object.assign(document.createElement("link"), { id: "rp3-fonts", rel: "stylesheet", href: FONTS_URL });
          ...
          <div className="rp-card">
            <div className="rp-error">
              ...
              <Link to="/forgot-password" className="rp-btn">
                ...
                <span className="rp-btn-txt">Demander un nouveau lien <i className="ri-arrow-right-line" /></span>
              </Link>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="rp-root">
      ...
      <div className="rp-left">
        <Link to="/login" className="rp-back"><i className="ri-arrow-left-s-line" /><span>Connexion</span></Link>
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .rp-root{min-height:100vh;background:#f3f3f9;display:flex;align-items:center;justify-content:center;font-family:var(--fb);color:var(--tx);overflow:hidden;padding:40px 24px;position:relative;}
  ...
  .rp-left{background:var(--bg2);background-image:linear-gradient(rgba(26,86,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(26,86,255,0.015) 1px,transparent 1px);background-size:10px 10px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:44px 40px;position:relative;z-index:10;border-radius:24px;max-width:460px;width:100%;box-shadow:0 40px 100px rgba(0,0,0,.70),0 0 0 1px rgba(26,86,255,.08) inset;}
  ...
  /* Brand */
  .rp-brand{text-align:center;margin-bottom:18px;}
  ...
  .rp-logo{width:44px;height:auto;filter:drop-shadow(0 0 14px rgba(26,86,255,.70));position:relative;z-index:2;}
  .rp-ring{position:absolute;inset:-5px;border-radius:50%;border:1px solid rgba(99,102,241,.22);animation:ring-p 3.2s ease-out infinite;}
  .rp-ring-2{inset:-16px;border-color:rgba(26,86,255,.12);animation-delay:1.1s;}
  .rp-ring-3{inset:-28px;border-color:rgba(99,102,241,.08);animation-delay:2.2s;}
  ...
  .rp-card::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(26,86,255,.5),transparent);}
  .rp-card::after{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%, rgba(26,86,255,.03) 0%, transparent 60%);pointer-events:none;}
  ...
  /* Alert */
  .rp-alert{display:flex;align-items:center;gap:10px;padding:10px 14px;background:rgba(26,86,255,.05);border:1px solid rgba(26,86,255,.12);border-radius:12px;margin-bottom:14px;}
  ...
  .rp-btn{width:100%;position:relative;height:54px;border:none;border-radius:14px;background:linear-gradient(135deg, #1A56FF 0%, #6366F1 50%, #0EA5E9 100%);background-size: 200% 200%;color:#030810;font-family:var(--fd);font-weight:700;font-size:15px;cursor:pointer;overflow:hidden;display:flex;align-items:center;justify-content:center;transition:all 0.4s cubic-bezier(.16,1,.3,1);box-shadow:0 8px 32px rgba(99,102,241,.25),0 0 0 1px rgba(255,255,255,.1) inset,0 0 20px rgba(26,86,255,.15);letter-spacing:.02em;margin-top:6px;animation: gradient-shift 8s ease infinite;text-decoration:none;}
  ...
  .rp-btn:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 12px 40px rgba(99,102,241,.40),0 0 0 1px rgba(255,255,255,.2) inset,0 0 30px rgba(26,86,255,.25);background-position: 100% 100%;color:#000;}
  ...
  @media(max-width:480px){.rp-left{padding:28px 20px;}.rp-card{padding:20px 16px;}.rp-title{font-size:18px;letter-spacing:.08em;}}
`;
