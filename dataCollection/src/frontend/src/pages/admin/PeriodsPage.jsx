...
function CreatePeriodModal({ onClose, onSave }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await periodService.create(year, month);
      onSave();
    } catch (err) {
        ...
        <div className="modal-content border-0 shadow">
          ...
          <div className="modal-body p-4">
            ...
            <div className="row g-3">
              <div className="col-6">
                <label className="form-label fw-medium">Année *</label>
                <input
                  type="number"
              ...
              <div className="col-6">
                <label className="form-label fw-medium">Mois *</label>
                <select
            ...
            <div className="alert alert-info mt-3 mb-0 py-2 fs-13">
              ...
              La période sera créée avec le statut <strong>Open</strong>.
              Seules les périodes ouvertes permettent les extractions.
            </div>
          </div>
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={handleSubmit} disabled={loading}>
              {loading ? (
                ...
                <><i className="ri-add-line me-1"></i>Créer la période</>
              )}
            </button>
          </div>
        </div>
...
export default function PeriodsPage() {
  ...
  const handleSave = () => {
    setShowCreate(false);
    showToast("Période créée avec succès.");
    loadPeriods();
  };
    ...
    <div className="page-content">
      <div className="container-fluid">
            ...
            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
              ...
              <div className="page-title-right">
                <ol className="breadcrumb m-0">
                  ...
                  <li className="breadcrumb-item active">Périodes</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <div className="alert alert-warning d-flex align-items-start gap-3 mb-4">
          ...
          <div>
            <strong>Règle métier RG-01 :</strong> Les extractions sont bloquées si la période
            courante est clôturée. Clôturer une période déclenche le dump mensuel automatique
            (archivage des lots REALTIME → MONTHLY + génération des snapshots KPI).
          </div>
        </div>
                      ...
                      {paginated.map((period) => (
                        <tr key={period.id}>
                          ...
                          <td className="text-muted fs-12">
                            {period.created_at
                              ? new Date(period.created_at).toLocaleDateString("fr-FR")
                              : "—"}
                          </td>
                          <td className="text-muted fs-12">
                            {period.closed_at
                              ? new Date(period.closed_at).toLocaleDateString("fr-FR")
                              : "—"}
                          </td>
                          <td>
                            {period.status === "open" ? (
                              ...
                              <span className="text-muted fs-12">
                                <i className="ri-check-line me-1"></i>Clôturée
                              </span>
                            )}
                          </td>
                        </tr>
      ...
      <ConfirmModal
        ...
        message={
          closeTarget
            ? `Vous allez clôturer la période ${closeTarget.year}/${String(closeTarget.month).padStart(2,"0")}. Les extractions seront bloquées (RG-01) et le dump mensuel sera déclenché.`
            : ""
        }
