    ...
    >
      <div className="card-body p-4">
          ...
          <div>
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <span className="opacity-50" style={{ fontSize: 11, color: '#cbd5e1' }}>
                {entity.n_periods} mois analysés
              </span>
            </div>
          </div>
        ...
        <div className="row g-2">
          {[
            { label: "Vélocité", value: latestVelocity.toFixed(1), suffix: "", trend: entity.velocity_trend, type: "velocity", icon: "ri-flashlight-line" },
            { label: "Revue", value: latestReview.toFixed(1), suffix: "h", trend: entity.review_trend, type: "review_time", icon: "ri-time-line" },
            { label: "Qualité", value: Math.round(latestQuality * 100), suffix: "%", trend: entity.quality_trend, type: "quality", icon: "ri-shield-check-line" }
          ].map((m, idx) => (
          ...
          <div className="d-flex align-items-center justify-content-between mb-4">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                ...
                <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>Analyse IA · Détection d'anomalies</span>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            {((user?.role === 'super_admin' || user?.role === 'site_manager' || user?.role === 'project_manager' || user?.role === 'viewer') && (
              <button
                ...
                style={{
                  ...
                  padding: '10px 16px',
                  borderRadius: '9px',
                  background: intelligenceView === 'sites' ? 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)' : 'transparent',
                  border: 'none',
                  ...
                  justifyContent: 'center',
                  gap: '8px',
                  boxShadow: intelligenceView === 'sites' ? '0 4px 12px rgba(79, 70, 229, 0.3)' : 'none'
                }}
              >
            ...
            {((user?.role === 'super_admin' || user?.role === 'team_lead' || user?.role === 'project_manager' || user?.role === 'viewer') && (
              <button
                ...
                style={{
                  ...
                  padding: '10px 16px',
                  borderRadius: '9px',
                  background: intelligenceView === 'teams' ? 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)' : 'transparent',
                  border: 'none',
                  ...
                  justifyContent: 'center',
                  gap: '8px',
                  boxShadow: intelligenceView === 'teams' ? '0 4px 12px rgba(79, 70, 229, 0.3)' : 'none'
                }}
              >
              ...
              <div>
                ...
                <div className="row g-3 mb-4">
                  {[
                    ...
                    { icon: "ri-link-m", label: "Corrélations", value: intelligenceData.correlations?.correlations?.length ?? 0, color: "#6366f1", bgGlow: "rgba(99, 102, 241, 0.08)" },
              ...
              <div>
                ...
                <div className="row g-3 mb-4">
                  {[
                    ...
                    { icon: "ri-link-m", label: "Corrélations", value: teamIntelligenceData.correlations?.correlations?.length ?? 0, color: "#6366f1", bgGlow: "rgba(99, 102, 241, 0.08)" },
...
export default function ComparativeAnalyticsPage() {
  ...
  const chartData = useMemo(() => {
    ...
    trends.forEach(t => {
      ...
      let val = t.metrics[activeMetricId];
      if ((activeMetricId === 'quality_score' || activeMetricId === 'merged_rate') && val != null && val <= 1.0) {
        val = val * 100;
      }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (trends.length > 0) {
        ...
        // 6. DORA benchmark insight
        if (doraData && doraData.length > 0) {
          const eliteSites = doraData.filter(d => d.dora_df_level === 'Elite' || d.dora_lt_level === 'Elite');
          const lowSites   = doraData.filter(d => d.dora_df_level === 'Low'   || d.dora_lt_level === 'Low');
          if (eliteSites.length > 0) {
            insightsArray.push({
              type: 'success',
              title: 'Standard DORA Elite Atteint',
              text: `${eliteSites.map(s => s.site_name).join(', ')} atteignent le standard de performance DORA Elite, classe mondiale selon les benchmarks Google DevOps Research & Assessment.`,
            });
          }
          if (lowSites.length > 0) {
            insightsArray.push({
              type: 'danger',
              title: 'Niveau DORA Insuffisant : Plan d\'Action Requis',
              text: `${lowSites.map(s => s.site_name).join(', ')} sont en zone Low selon les standards DORA. Un plan d'acceleration CI/CD et de reduction du Lead Time doit etre engage en priorite.`,
            });
          }
        }
      }
  ...
  const chartOptions = {
    ...
    yaxis: {
      labels: {
        formatter: (val) => val.toFixed((activeMetricId === 'quality_score' || activeMetricId === 'merged_rate') ? 0 : 1) + ((activeMetricId === 'quality_score' || activeMetricId === 'merged_rate') ? '%' : '')
      }
    },
    tooltip: {
      shared: true,
    ...
    annotations: {
      yaxis: [{
        y: activeMetricId === 'velocity' ? 4 : ((activeMetricId === 'quality_score' || activeMetricId === 'merged_rate') ? 85 : 24),
        borderColor: '#9ca3af',
        label: {
          borderColor: '#9ca3af',
  ...
  return (
    <div className="page-content">
      ...
      {loading ? (
        ...
        <div className="card border-0 shadow-sm mb-4 bg-primary text-white" style={{ borderRadius: 16, background: "linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%)" }}>
        ...
        <div className="col-lg-9">
          ...
          {loading ? (
            ...
            <div className="card-header bg-white border-0 p-4 d-flex align-items-center justify-content-between">
              ...
              <div className="d-flex align-items-center gap-3">
                 <span className="d-flex align-items-center gap-2 px-3 py-2 rounded-3" style={{background: '#f0fdf4', border: '1px solid #bbf7d0'}}>
                    <span style={{width: 7, height: 7, background: '#22c55e', borderRadius: '50%', boxShadow: '0 0 0 3px #dcfce7'}}></span>
                    <span className="fw-bold text-success" style={{fontSize: 11, letterSpacing: '0.05em'}}>LIVE</span>
                 </span>
          ...
          <div className="row mt-4">
            <div className="col-12">
              <div className="card border-0 shadow-sm overflow-hidden" style={{borderRadius: 16}}>
                <div className="card-header bg-white border-0 p-4 d-flex align-items-center justify-content-between">
                  <div className="d-flex align-items-center gap-3">
                    <div style={{width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", background: "#EEF2FF", borderRadius: 10, flexShrink: 0}}>
                       <i className="ri-speed-up-line text-primary fs-4"></i>
                    </div>
                    <div>
                            ...
                            <div key={site.site_id} className="col-md-4">
                              <div className="p-4 rounded-4 border border-light-subtle h-100 transition-all hover-shadow-sm" style={{background:"#FFFFFF"}}>
                                <div className="d-flex align-items-center justify-content-between mb-4 pb-2 border-bottom border-light">
                                  ...
                                  <span className="text-muted fs-11 fw-bold"><i className="ri-calendar-2-line me-1"></i>{site.period_label}</span>
                                </div>
                                
                                <div className="mb-4">
                                  <div className="d-flex align-items-center justify-content-between mb-2">
                                    <div className="d-flex align-items-center gap-2">
                                      <i className="ri-git-branch-line text-muted fs-14"></i>
                        ...
                        <div className="d-flex flex-wrap gap-4">
                          {[
                            ...
                            <div key={l.level} className="d-flex align-items-center gap-2">
                              <span className="fw-bold fs-10 px-2 py-1" style={{background: l.color+'18', color: l.color, border: `1px solid ${l.color}55`, borderRadius: 6, letterSpacing: '0.03em'}}>{l.level}</span>
                              <small className="text-muted fs-10">DF: {l.df} · LT: {l.lt}</small>
                            </div>
                          ))}
                        </div>
                      </div>
